...
- Hardware languages, HLS, DSLs, and accelerator frameworks: `Chisel`, `BSV`, `Filament`, `VitisHLS`, `LegUp`, `Dynamatic`, `AutoDSE`, `ScaleHLS`, `dahlia2020`, `spatial2018`, `aetherling2020`, `assassyn2025`;
- Hardware IR and infrastructure: `calyx2021`, `firrtl2017`, `hector2022`, `circt2021`;
- Local completed-work keys for the detailed technical chapters: `cement2024`, `cement2_2025`, `clay2025`, `skyegg2026`, `cayman2025`.
...
Assassyn adds a related but distinct line: high-level architecture simulation and low-level hardware implementation should not be maintained as disconnected codebases. Its asynchronous-event abstraction describes pipeline stages and architectural behavior once, then uses the same program object for cycle-accurate simulation and RTL generation. For Chapter 1, this is useful evidence that architecture-to-hardware co-design is not only an HDL-versus-HLS abstraction choice. For Chapter 5, it should be positioned as external related work for future direction 1 rather than as part of the author's existing work. Its limitation is that the unified abstraction is still mainly a human-authored programming model; it does not make agent-oriented open-intent decomposition, candidate generation, diagnosis, repair, and experience consolidation the central research object.
...
The source papers repeatedly frame compiler infrastructure as the layer that keeps software semantics available long enough for hardware-aware decisions. APS emphasizes compiler support for custom instructions through pattern matching and bitwidth-aware vectorization. Aquas moves this further by using MLIR to represent both hardware-side ISAX behavior and software programs at compatible abstraction levels, then using an e-graph-based matching mechanism to overcome syntactic divergence and premature lowering. ISAMORE treats e-graphs and anti-unification as the means to discover semantically reusable instruction patterns rather than only syntactically similar hotspots. TAIDL and ACT should extend this compiler-interface family in future-work discussion: TAIDL turns tensor-accelerator ISA facts into a definition language and scalable test oracles; ACT uses tensor-accelerator ISA descriptions to generate compiler backends. They support the architecture-chip-software co-generation argument, while also showing that current pipelines still assume a hand-written target description.
...
For the proposal introduction, this family should not claim that LLMs solve chip design. The correct synthesis is: agentic methods become relevant when they are embedded in a collaboration mechanism among model, tool, target constraint, execution result, formal or executable oracle, and structured records. Hardware design and compiler optimization require stricter constraints than ordinary software editing because errors can involve protocol violations, timing failure, semantic miscompilation, area and power regressions, or non-reproducible performance. AccelOpt and Autocomp should be used as boundary cases for the agentic compiler-infrastructure future direction: both show that LLM-driven or agentic search can improve accelerator kernels or tensor-accelerator code when correctness/performance evaluation and optimization memory are available, but neither should be described as complete compiler generation.
