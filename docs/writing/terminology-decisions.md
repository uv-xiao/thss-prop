...
| agile chip design | 中文为主 | 敏捷芯片设计 | 敏捷芯片设计（agile chip design） | 与报告题目一致，强调快速形成、验证、映射和迭代硬件能力。 | 敏捷芯片开发 | 待确认 |
| architecture interface | 中文为主 | 架构接口 | 架构接口（architecture interface） | 已在第一章定义，覆盖 ISA、ISAX、存储/互连和运行时可见硬件能力。 | 体系结构接口 | 待确认 |
| compilation interface | 中文为主 | 编译接口 | 编译接口（compilation interface） | 已定义为软件语义进入 IR、lowering、调度、代码生成和反馈的边界。 | 编译器接口 | 待确认 |
| compiler infrastructure | 中文为主 | 编译基础设施 | 编译基础设施（compiler infrastructure） | 比“编译器基础设施”更适合包含 IR、pass、证据、后端和运行时接口。 | 编译器基础设施 | 待确认 |
| agentic method | 保留英文 | agentic 方法 | agentic 方法 | “智能体式”可解释但不够稳定；当前报告与工业报告常用 agentic。 | 智能体式方法 | 待确认 |
| agent harness | 保留英文 | agent harness | agent harness（智能体协作框架） | harness 暂无稳定译法，且项目内有明确技术含义。 | 智能体框架、代理框架 | 待确认 |
| evidence memory | 中文为主 | 证据记忆 | 证据记忆（evidence memory） | 强调结构化 facts/events/artifacts，不是聊天历史。 | 经验库、记忆库 | 待确认 |
| target facts | 中文为主 | 目标事实 | 目标事实（target facts） | IntelliC/编译 harness 语境中的目标架构事实。 | 目标信息 | 待确认 |
...
| typed artifact | 双语固定搭配 | 类型化产物 | 类型化产物（typed artifact） | “artifact”在报告中应落到可解析、可重放对象。 | 类型化制品 | 待确认 |
| executable oracle | 双语固定搭配 | 可执行 oracle | 可执行 oracle（executable oracle） | oracle 在验证语境中常保留英文；“可执行判定器”不够自然。 | 可执行判定器 | 待确认 |
...
| action | 保留英文/待定 | action | action（动作） | Clay/IntelliC 中是系统对象；直接译为“动作”可能丢失专有含义。 | 动作 | 待确认 |
| fixed action | 保留英文 | fixed action | fixed action | IntelliC 内部分类，不宜翻译成“固定动作”。 | 固定动作 | 待确认 |
| agent action | 保留英文 | agent action | agent action | IntelliC 内部分类，与 fixed action 对应。 | 智能体动作 | 待确认 |
| e-graph | 保留英文 | e-graph | e-graph | 领域通用术语，中文“等价图”可能与具体数据结构不完全等同。 | 等价图 | 待确认 |
| equality saturation | 保留英文/双语 | equality saturation | equality saturation（等式饱和） | 中文可辅助理解，但后文建议保留英文。 | 等式饱和 | 待确认 |
| EqSat | 保留英文 | EqSat | EqSat | equality saturation 的常用简称。 | 等式饱和 | 待确认 |
...
| motif | 保留英文/待定 | motif | rewrite motif（重写 motif） | “模式”容易与 pattern 混淆；建议保留 motif。 | 模式、母题 | 待确认 |
| proof-derived motif | 双语固定搭配 | proof-derived motif | proof-derived motif（证明派生 motif） | EggMind 专有机制，保留英文便于对应论文。 | 证明派生模式 | 待确认 |
| tractability guidance | 中文为主 | 可处理性引导 | 可处理性引导（tractability guidance） | 比“可 tractable 指导”更适合中文报告。 | 可解性指导、可控性引导 | 待确认 |
| simplification hint | 中文为主 | 简化提示 | 简化提示（simplification hint） | EggMind 机制，中文自然。 | 简化线索 | 待确认 |
| phase | 中文为主 | 阶段 | 阶段（phase） | 一般语境可翻译；EggMind/RIMT 可首次括注。 | 相位 | 待确认 |
| anti-unification | 保留英文/双语 | anti-unification | anti-unification（反合一） | “反合一”可解释，但术语读者可能更认英文。 | 反统一 | 待确认 |
| e-class | 保留英文 | e-class | e-class | e-graph 数据结构对象，保留英文。 | 等价类 | 待确认 |
| e-node | 保留英文 | e-node | e-node | e-graph 数据结构对象，保留英文。 | 等价节点 | 待确认 |
| mapping e-node | 保留英文 | mapping e-node | mapping e-node | SkyEgg 专有对象，保留英文清晰。 | 映射等价节点 | 待确认 |
| extraction | 保留英文/待定 | extraction | extraction（抽取） | e-graph 领域常用英文；中文“抽取”可首次解释。 | 提取 | 待确认 |
| custom instruction | 中文为主 | 自定义指令 | 自定义指令（custom instruction） | ASIP/ISAX 语境稳定。 | 定制指令 | 待确认 |
| custom instruction extension / ISAX | 双语固定搭配 | 自定义指令扩展 / ISAX | 自定义指令扩展（ISAX） | ISAX 是论文常用对象，正文可交替使用。 | 指令集扩展 | 待确认 |
| ASIP | 保留英文 | ASIP | 应用专用指令集处理器（ASIP） | 首次中文展开，后文用 ASIP。 | 专用指令集处理器 | 待确认 |
...
| action | 保留英文/待定 | action | action | Clay interface 中 action 是接口原语；建议保留英文。 | 操作、动作 | 待确认 |
...
| high-level synthesis / HLS | 中文为主 | 高层综合 / HLS | 高层综合（high-level synthesis, HLS） | 中文稳定，后文可用 HLS。 | 高级综合 | 待确认 |
...
| event layer | 双语固定搭配 | event layer | event layer（事件层） | Cement 专有层名，建议保留英文并解释。 | 事件层 | 待确认 |
| control sub-language / ctrl | 双语固定搭配 | ctrl 子语言 | ctrl 子语言（control sub-language） | Cement 原文对象。 | 控制子语言 | 待确认 |
| timing analysis | 中文为主 | 时序分析 | 时序分析（timing analysis） | 中文稳定。 | 时间分析 | 待确认 |
...
| whole-application program structure tree / wPST | 双语固定搭配 | 全应用程序结构树 / wPST | 全应用程序结构树（whole-application program structure tree, wPST） | Cayman 核心表示，首次中文展开。 | 整程序结构树 | 待确认 |
...
| benchmark | 保留英文 | benchmark | benchmark | 中文“基准测试”可解释，但正文大量出现时英文更自然。 | 基准测试 | 待确认 |
| workload | 保留英文/待定 | workload | workload（工作负载） | 中文“工作负载”自然；是否全译待定。 | 工作负载 | 待确认 |
| runtime | 保留英文/待定 | runtime | runtime（运行时） | “运行时”中文自然，但框架名/系统层常保留英文。 | 运行时系统 | 待确认 |
| graph break | 保留英文 | graph break | graph break | PyTorch/Dynamo 专有术语，建议保留。 | 图断裂 | 待确认 |
| backend | 保留英文/待定 | backend | backend（后端） | 编译器后端可译，系统组件可保留。 | 后端 | 待确认 |
