...
- 来自原文或 `resources/` 的图片在 `report/assets/` 中一律使用 PDF 格式，正文不得引用 PNG、JPG、SVG 等原文图片格式。若源文只有非 PDF 图片，应先转换或重新导出为 PDF；唯一例外是由 GPT 自动绘制的新图。
...
- 如果源文组图同时包含图片和源码片段，使用 Typst `grid` 在一个父 `#figure` 中重组完整组图；源码片段使用 `codly` 装饰代码块，不要把源码改写成普通段落或拆成独立图号。
- 原文算法、递归过程、调度器和优化流程应使用 Typst `algorithmic` 呈现为伪代码。除非原文就是符号定义表或实验表，不要用普通表格代替伪代码。
...
若源文已经有完整组合 PDF，应优先直接复制并插入完整组图，而不是用上述模板重新拼接；若只有 PNG/JPG/SVG 等格式，先转换为 PDF 后再引用。
...
- 当前状态：已补入 overview、EqSatL、proof memory 和 Isaria comparison 图；正文强调“智能体方法 + 形式化策略表示 + 验证约束体系”，不写成自由代码生成。
