...
#let cover-page-normal(
  ...
  set page(
    "a4",
    margin: (top: 2cm, bottom: 2cm, left: 2.6cm, right: 2.6cm),
    header: none,
    footer: none,
  )
  set align(center + top)
  ...
  box(
    grid(
      ...
      [
        #set align(center + horizon)
        #image("../pkuword.svg", height: 1.85em, fit: "contain")
      ],
    ),
  )
  v(1.25cm)
  text(字号.小二, font: 字体.黑体)[
    #show: show-cn-fakebold
    #strong(cthesisname)
  ]
  v(2.05cm)
  context {
    show: show-cn-fakebold
    set text(字号.小一, font: 字体.黑体)
    set par(leading: 1.32em)
    let forced-title-parts = ctitle.split("\n")
    let title-parts = if forced-title-parts.len() > 1 {
      forced-title-parts
    } else {
      split-text-by-width(ctitle, 11.6cm)
    }
  ...
  let field-contents = ()
  for (name, value) in cover-fields {
    field-contents.push([
