...
#figure(
  ...
  block(breakable: false)[
    #text(size: 7.4pt)[
      #table(
        columns: (0.55fr, 5.1fr),
        inset: 2.4pt,
        align: horizon,
        table.header([步骤], [候选选择递归过程]),
        [输入], [wPST `T`，性能剖析和程序分析结果 `R`],
        [输出], [满足面积约束下的帕累托最优候选选择],
        [1], [`DP(v)`: 若 `prune(v, R)` 成立，说明该区域缺少加速价值或不可有效综合，直接返回空解],
        [2], [若 `v` 是基本块区域，则令 `F[v] <- filter(pareto(accel(v, R)))`，即为该基本块生成可行加速配置并保留帕累托前沿],
        [3], [否则初始化 `F[v] <- empty`；对每个子节点 `u in v.children` 递归执行 `DP(u)`，并以 `F[v] <- filter(F[v] tensor F[u])` 组合不同子树的候选解],
        [4], [若 `v` 是控制流区域，则进一步令 `F[v] <- filter(F[v] union pareto(accel(v, R)))`，在“加速整个控制流区域”和“组合其子区域”之间选择帕累托解],
        [5], [从 `T.root` 开始调用 `DP(T.root)`，最终返回 `F[T.root]` 作为全应用候选选择结果],
      )
    ]
  ],
  caption: [Cayman 源文 Algorithm 1 的候选选择算法。该动态规划在 wPST 上自底向上组合候选，同时用剪枝、帕累托前沿和过滤控制搜索规模。],
)
