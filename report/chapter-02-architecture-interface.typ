...
ISAMORE 将可复用自定义指令识别建模为跨程序模式发现问题，并提出 RIMT 方法。其输入是一组领域代表程序，输出是一组在性能和面积之间形成帕累托权衡的自定义指令候选。这个过程首先把 LLVM IR 转换为结构化领域专用语言，再编码为等价图；随后通过分阶段等式饱和和等价图反合一发现语义等价空间中的公共模式；最后用硬件感知代价模型选择值得实现的模式，并通过 HLS 和 RoCC 包装后端生成可评估硬件。与 APS/Aquas 相比，ISAMORE 的位置更靠前：它不只是让编译器使用已有 ISAX，而是帮助设计者决定哪些 ISAX 值得进入后续工具链。
...
为了让一般程序能够进入等价图反合一，ISAMORE 首先设计结构化领域专用语言。该语言覆盖算术、逻辑、访存等常规操作，同时引入 `Loop` 和 `If` 表达控制流，并提供 `Vec`、向量一元/二元/三元运算、模式变量和 `App` 等结构。`Loop` 以类似 do-while 的风格表达循环携带变量、循环条件和循环体结果，使控制流能够以结构化、数据流中心的方式进入等价图。该语言还具有强类型系统，利用等价类分析推导每个等价类的结果类型，并对 `If` 和 `Loop` 的输入输出元组施加结构约束。这个设计解决了直接把一般程序放入等价图的第一层障碍：程序不再只是若干代数表达式，而是保留了循环、条件、类型和向量结构的可泛化语义对象 @isamore2026。

在此基础上，ISAMORE 的核心方法是等价图反合一。等价图用等价类表示等价项集合，等式饱和通过重写规则暴露语义等价形式；反合一则在两个项之间求最小公共泛化，从而发现公共结构。例如，两个语法不同但可重写到同一等价形式的表达式，可以被泛化为带模式变量的候选指令模式。这个思路把“复用”从语法匹配提升到语义等价空间：同一个自定义指令可以覆盖多个表面形态不同的程序片段，只要它们在等价图中共享可泛化结构。

=== 分阶段识别流程与可扩展反合一

直接使用朴素等价图反合一并不可行，因为真实程序的等价类数量和候选反合一模式数量会迅速爆炸。ISAMORE 因此提出分阶段迭代。它放弃一次性对所有重写规则做完整饱和，而是把规则按饱和型/非饱和型、整数/浮点、向量/标量等维度组织为基础规则集，并由阶段调度器逐步选择规则集。早期阶段先应用不会显著增加等价类数量的饱和型规则，尽可能暴露低成本等价形式；后续阶段再以受限次数应用非饱和型规则，以避免等价图规模失控。每个阶段中，RIMT 会把以前识别的模式作为模式应用重写重新注入等价图，使后续阶段能够在已有模式的基础上进一步泛化。这样做既控制了搜索规模，又允许指令模式从局部公共结构逐渐演化为更高复用度的候选 @isamore2026。

RIMT 的第二个关键是智能反合一。朴素反合一需要枚举大量等价类对，并对每对等价节点递归生成候选模式，实际复杂度很快失控。智能反合一用两类启发式降低复杂度。第一是基于相似度的等价类配对：ISAMORE 用类型信息排除结果类型不一致的等价类，再通过结构哈希估计等价类的结构相似度，只对相似度超过阈值的配对做反合一。结构哈希对字面量、参数和模式变量做统一处理，强调结构而非具体常量，从而更适合寻找可复用模式。第二是启发式反合一模式采样：当某个等价节点对的反合一候选过多时，ISAMORE 不保留全部笛卡尔积，而用边界或 kd-tree 策略根据延迟/面积特征选取代表性模式。前者保留特征值极端的模式以提高效率，后者在特征空间中取更广覆盖以改善质量。二者共同使等价图反合一从理论上有吸引力但不可用的方法，变成可以处理真实程序的指令识别机制 @isamore2026。
...
除了语义复用，ISAMORE 还把数据级并行纳入指令发现过程。传统指令发现往往把标量模式作为主要对象，即使程序中存在多路相似操作，也不一定生成向量化自定义指令。ISAMORE 的模式向量化把一个向量化模式看作同一个模式在多个通道上的应用。其流程先通过智能反合一找到可重复出现的标量种子，再把同一基本块中的多个种子打包成 `Vec` 节点；随后通过提升重写恢复 VecAdd、VecMul 等向量构造，并用耦合重写建立向量到标量通道的数据流；最后用贪心提取和压缩式剪枝消除 `Get-Vec-Get` 环与组合式打包爆炸，只保留高数据级并行度的非重叠向量化方案。这使自定义指令发现不局限于“找到一个可复用标量片段”，而能同时探索向量化、硬件循环和更高吞吐的数据通路 @isamore2026。
...
ISAMORE 的基准测试评估展示了这种方法的可扩展性和收益。在九个来自计算机视觉、机器感知、数字信号处理和密码学的内核上，朴素等价图反合一因候选模式爆炸而在所有案例中超过 30GB 内存限制；启用 RIMT 后，所有基准测试均在 145 秒内完成，内存不超过 799MB，峰值等价图规模相比朴素方法降低 6 到 39 倍。在性能上，ISAMORE 相比 NOVIA 等语法合并方法获得更高加速比和更低面积开销；启用向量化后，ISAMORE 相对 NOVIA 的优势平均达到 1.76x，最高达到 2.69x，并在多个基准测试中展示模式向量化对性能的进一步贡献。MatChain 案例还表明，结构化领域专用语言编码控制流后，ISAMORE 可以识别可复用硬件循环，生成循环流水加速器，获得远超基本块级方法的优化粒度 @isamore2026。

更重要的是，ISAMORE 在真实开源库和具体硬件实例中体现了“可复用架构能力”的价值。对 liquid-dsp、CImg 和 PCL 等库的研究显示，语法合并方法往往生成巨大且低复用的硬件单元，而 ISAMORE 能识别多个更小、更高复用的自定义指令。在 CImg 中，NOVIA 生成一个规模为 167 的大硬件单元且只合并少量基本块，ISAMORE 则识别八条平均复用 93 次的指令，获得 1.18x 加速比，同时面积仅为 975 平方微米，相比 NOVIA 节省 90.5% 面积。在 PCL 模块中，ISAMORE 相比 NOVIA 平均获得 1.64x 加速比，最高 2.73x，并节省 93.2% 面积。这些数据说明，可复用性不是附加指标，而是决定自定义指令是否适合进入 ISA 的核心标准 @isamore2026。

ISAMORE 还在量化 LLM 和后量子密码中展示了从自动识别到真实硬件集成的可行性。对 BitNet b1.58 的 BitLinear 推理实现，ISAMORE 从 `bitnet.cpp` 风格实现中识别出打包低比特点积的向量化模式，并生成带 RoCC 包装器的自定义硬件单元；在 Rocket tile 上通过 Verilator RTL 仿真获得 2.15x BitLinear 加速比，OpenROAD 报告 4.81% 面积开销且无频率下降。对 CRYSTALS-KYBER 的 NTT 实现，ISAMORE 自动识别正向 NTT 和逆向 NTT 复用的蝶形定制指令，并在 Rocket tile 上获得 5.15x 加速比，面积开销主要来自硬件乘法器。这些案例证明，ISAMORE 不只是静态分析工具，而能够与 RISC-V/SoC、HLS、RTL 仿真和物理设计流程连接，进入实际敏捷芯片设计流程 @isamore2026。

从本章主线看，ISAMORE 对 APS/Aquas 形成了关键补充。APS/Aquas 使一个已知定制能力能够被架构接口承载、被硬件工具实现、被编译器调用并被系统评估；ISAMORE 则反过来从领域程序集合中发现哪些能力值得进入这一流程。其方法创新在于把等价图等式饱和、反合一、结构化控制流领域专用语言、分阶段搜索、智能反合一、模式向量化和硬件感知选择结合起来，使“可复用自定义指令”成为可自动发现、可量化权衡、可硬件落地的敏捷芯片设计机制。由此，领域定制不再完全依赖专家凭经验挑选热点，而可以通过程序语义、硬件代价和跨工作负载复用共同驱动。
