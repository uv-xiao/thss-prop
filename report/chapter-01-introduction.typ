...
因此，领域定制计算的趋势可以概括为两个转变。第一，硬件能力从通用核外的单点加速器，转向可通过架构接口表达、可由编译器发现和使用、可在不同平台迁移的系统能力。第二，优化对象从单个内核或单个模块，转向应用语义、存储层次、指令/微架构机制、编译映射和运行时反馈的联合空间。这个趋势为“敏捷芯片设计”提出了更高要求：硬件创新不仅要能快速生成，还要能快速验证、映射、复用和迭代。
...
AI 辅助设计和 agentic 方法为生产力问题提供了新的可能。AlphaEvolve 将 LLM 组织为进化式代码智能体，通过直接修改代码并接受评价器反馈来迭代改进算法，在 Google 的计算基础设施优化和算法发现任务中展示了“模型 + 工具 + 评价器”闭环的潜力 @alphaevolve2025。NVIDIA AVO 进一步把类似思想推进到性能敏感的内核与编译优化：它用自主代码智能体 替代固定变异、交叉和手工启发式，在 Blackwell B200 GPU 的注意力内核优化中利用谱系、领域知识和执行反馈 生成、修复、批判和验证实现修改 @avo2026。这些工作说明，agentic 方法的关键不只是模型能生成代码，而是模型能在可执行评价、领域知识和反馈约束中持续改进产物。

然而，芯片设计比一般软件工程和单一内核优化更强调语义正确性、时序约束、接口协议、可综合性和长期维护。直接让模型生成 RTL、编译 pass 或调度策略，容易把设计意图、目标约束和验证结果混在自然语言或临时代码中，难以审计和复用。因此，敏捷芯片设计中的 agentic 自动化不能脱离架构接口、硬件 IR、编译基础设施和验证 oracle。它需要被放在可解释、可检查、可回放的系统中，成为受约束的协同机制，而不是不可控的代码生成器。
...
在这个生态中，架构接口首先定义“硬件能够提供什么能力”。它不仅包括 ISA 或 ISAX，也包括微架构机制、存储层次、互连、加速器执行模型和运行时可见资源抽象。一个领域能力若不能通过架构接口稳定表达，就难以被编译器发现和利用；一个架构接口若只描述指令格式而不描述访存路径、执行约束和系统边界，也很难支撑复杂数据密集型应用。APS 和 Aquas 的问题意识正来自这一点：领域定制处理器需要统一扩展接口、硬件合成机制和编译支持，而复杂 ISAX 还需要把访存接口属性、缓存效应、scratchpad 和事务粒度等目标事实纳入协同设计 @aps2025 @aquas2026。

编译接口则定义“软件如何发现、使用、优化和验证这些能力”。它覆盖中间表示、逐层降低、模式/重写、调度、代码生成、运行时接口和性能剖析反馈等层次。编译器在这里不是后端附属工具，而是连接应用语义、架构能力、硬件实现和运行时行为的中间层。ISAMORE 通过等价图反合一在程序等价空间中发现可复用指令模式，SkyEgg 将代数变换、硬件映射和调度放入统一等价图设计空间，EggMind 进一步尝试把大模型的策略生成能力放入形式化可控的等式饱和策略对象中 @isamore2026 @skyegg2026 @eggmind2026。这些工作共同说明，编译接口不仅决定“能否生成代码”，还决定“哪些硬件能力能被系统性发现、证明和复用”。
...
这一接口的核心未解问题在于“可复用表达”。许多领域定制工作仍依赖专家从热点代码或特定内核中抽取定制能力，容易得到过度专用的硬件单元；不同处理器扩展接口、访存路径和执行模型之间也缺少统一抽象，使同一能力难以跨 SoC、跨应用和跨编译工具链迁移。已有 ASIP 框架开始尝试把接口、合成和编译支持放入端到端流程，但复杂数据密集应用还要求设计者同时刻画缓存效应、scratchpad、事务粒度和内存带宽约束；可复用自定义指令识别则进一步要求从程序等价空间中发现具有跨程序价值的语义模式，而不是只合并局部语法相似片段 @aquas2026 @isamore2026。
...
这一接口的难点在于同时满足语义保真、优化空间规模和目标适配。过早降低会丢失循环、代数结构、数据布局和领域约束，使后端只能在低层表示上做局部优化；而过于宽泛的重写空间又会造成搜索爆炸，难以保证策略可迁移、结果可解释。e-graph 相关方法的价值在于把语义等价表达为可检索、可抽取、可证明的空间，但它仍需要目标事实、代价模型和策略控制，否则容易形成“正确但不可用”或“可用但不可扩展”的优化过程 @isamore2026 @eggmind2026。因此，编译接口的研究重点正在从“能否完成一次降低”转向“能否把目标约束、语义证据和优化策略组织成可复用基础设施”。

第四类研究关注编译到运行时的接口，即编译决策如何进入异构执行、调度、数据移动和反馈循环。深度学习框架已经清楚展示了这一趋势。PyTorch `torch.compile` 和 TorchDynamo 通过 Python 帧求值 捕获动态图程序，将可编译区域交给后端，同时以 graph break、recompilation limit 和 eager fallback 处理动态语言与静态编译之间的边界 @pytorchCompile2026。NVIDIA TensorRT-LLM 则把 LLM 推理优化组织为模型构建、引擎生成、内核融合、量化、KV cache、连续/在途批处理、分页注意力、C++ 运行时和服务后端 的联合系统 @tensorrtllm2026。

这类系统说明，编译和运行时之间已经不存在清晰的单向分工。编译器需要理解运行时批处理、缓存、通信、内存容量和延迟/吞吐权衡；运行时也需要依赖编译器生成的 引擎、内核、布局 和调度策略。工业级异构架构进一步放大了这一问题：多 GPU、多 NPU、片间互连、分布式内存、专家并行、令牌分发、集合通信 和服务化资源管理共同决定最终性能。由此，编译到运行时的研究挑战不只是生成更快内核，而是建立能够跨硬件层次携带目标事实、执行证据和反馈信号的协同接口。
...
更进一步，AlphaEvolve 与 NVIDIA AVO 说明 agentic 方法正在进入更接近算法发现、编译优化和高性能内核 搜索的区域。AlphaEvolve 用 LLM 修改代码，并通过 评价器 反馈进行进化式搜索；AVO 则把 自主代码智能体 用作 变异算子，在 NVIDIA Blackwell B200 GPU 的注意力内核优化中结合 谱系、领域知识和执行反馈 生成、修复、批判和验证实现修改 @alphaevolve2025 @avo2026。它们共同表明，agentic 方法的技术核心不是“模型更会写代码”，而是模型能否被放入目标约束、工具调用、执行反馈和证据记忆组成的闭环中。
...
第二章首先围绕“应用需求如何通过架构接口转化为可复用的指令、微架构和加速器能力”展开。APS 和 Aquas 关注应用专用指令集处理器与定制指令扩展的端到端协同，分别从开放 RISC-V 扩展接口、硬件合成、编译支持和复杂数据密集指令的访存接口属性、缓存影响、鲁棒编译映射等方面，回答领域能力如何被接入完整工具链 @aps2025 @aquas2026。ISAMORE 则进一步把问题推进到“定制什么”的层面，通过等价图反合一从程序等价空间中发现可复用定制指令模式，使定制指令不只是局部热点加速，而成为可跨程序复用的架构机制 @isamore2026。Cayman 将在这一章作为领域定制与端到端协同的重要组成部分，补充微架构约束与架构能力选择之间的联系。
...
@problem-chapter-map 概括了上述关系。第二至第四章分别以已有工作回应三个核心科学问题，并在相邻层次之间形成支撑关系；第五章则把这些问题重新组合为面向博士阶段后续研究的三个方向。这样的组织方式有两个目的：一方面，它避免把已有成果写成论文清单，而是说明每项工作在整体研究链条中的位置；另一方面，它也明确未来研究不是对已有工作的简单延长，而是从单点方法走向跨层协同系统。
