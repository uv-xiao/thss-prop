...
前期编译基础设施材料和种子页给出了这一方向的核心论证。面向新硬件目标的编译器需要同时接受两类输入，一类是程序员希望使用的编程界面，另一类是目标架构、存储层次、指令语义、布局约束、通信拓扑和执行边界等目标事实。二者之间缺少的中间层并非单个后端文件，而是一套由中间表示、编译遍、求解器、检查、证据记录和后端产物构成的编译基础设施。XLA、Arknife、ACT 和空间映射编译器等材料说明，目标事实可以以分层中间表示、架构/布局抽象、指令集驱动后端生成和空间时间映射搜索等不同方式进入编译器。智能体适合参与这些结构化对象的构造和演化，但不适合在缺少语义和目标契约的情况下自由生成编译器。
