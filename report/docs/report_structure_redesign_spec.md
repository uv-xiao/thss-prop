...
4. 该边界为第五章三个未来研究方向提供问题定义：可解释编译基础设施、验证约束下的协同生成、面向异构任务图的编译-运行时协同。
...
- 未来工作主要是三个概括性研究方向：可解释、可审计的编译基础设施；验证约束下的软硬件协同生成；面向异构任务图的编译-运行时协同，以及它们之间的集成验证。
...
#### 5.4 面向异构任务图的编译-运行时协同

按同样五段结构。

重点：

- 可以在本节中说明 PTO Runtime distributed features 作为该方向的已有基础或原型，但小节论证对象是“面向异构任务图的编译-运行时协同”。
...
3. 未来工作已确认只保留三个概括性方向：可解释、可审计的编译基础设施；验证约束下的软硬件协同生成；面向异构任务图的编译-运行时协同。IntelliC、Spine、PTO Runtime distributed features 只作为已有基础或原型出现。
