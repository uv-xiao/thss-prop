...
1. 应用需求如何稳定转化为可复用的架构、指令和微架构能力，而不是停留在单点接口和单程序热点；
2. 架构能力如何高质量落到硬件实现，并兼顾表达能力、确定性、性能和资源质量；
3. 编译、生成与运行时系统如何跨硬件架构建立可解释、可验证、可复用的证据闭环。
...
1. 领域定制与端到端协同：主要回应问题 1，即应用需求如何转化为可复用架构、指令和微架构能力；同时通过 ISAMORE 等工作支撑问题 3 中的形式化优化与证据闭环。
2. 硬件前端与综合优化：主要回应问题 2，即架构能力如何高质量落到硬件实现；同时通过 SkyEgg、Clay、HECTOR 等工作支撑问题 3 中的可验证编译/综合基础。
3. 大模型与形式化技术驱动的软硬件协同方法：主要回应问题 3，即编译、生成与运行时系统如何跨硬件架构建立可解释、可验证、可复用的证据闭环；同时反向支撑问题 1、2 中的定制能力发现和综合优化。
