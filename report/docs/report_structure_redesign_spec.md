...
- 必须明确区分两个关键接口：
  ...
  - 编译接口：把软件语义转化为可映射、可优化、可验证的执行形式，包括 IR、lowering、pattern/rewrite、调度、代码生成、runtime API、profiling feedback 等。
...
3. 未来工作已确认只保留三个概括性方向：验证约束下的软硬件协同生成；可解释、可审计的编译基础设施；面向工业级异构架构的算子优化与运行时编译协同。IntelliC、Spine、PTO Runtime distributed features 只作为已有基础或原型出现；`resources/seed-proposal.docx` 内容纳入第三个方向。
