...
4. 该边界为第五章三个未来研究方向提供问题定义：可解释编译基础设施、验证约束下的协同生成、面向工业级异构架构的算子-运行时编译协同。
...
- 未来工作主要是三个概括性研究方向：可解释、可审计的编译基础设施；验证约束下的软硬件协同生成；面向工业级异构架构的算子-运行时编译协同，以及它们之间的集成验证。
...
- 阶段 3：面向工业级异构架构的算子-运行时编译协同原型，包括长程 Agent Harness、核心 fused 算子优化、Torch 算子覆盖、性能反馈和回归验证；
- 阶段 4：跨层案例整合与论文/开源输出；
...
3. 未来工作已确认只保留三个概括性方向：可解释、可审计的编译基础设施；验证约束下的软硬件协同生成；面向工业级异构架构的算子-运行时编译协同。IntelliC、Spine、PTO Runtime distributed features 只作为已有基础或原型出现；`resources/seed-proposal.docx` 内容纳入第三个方向。
