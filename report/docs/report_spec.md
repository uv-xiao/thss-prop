...
算力紧缺不是单纯的硬件峰值性能问题，而是“算力需求增长”和“芯片/编译/软件生态生产力不足”共同作用的系统问题。领域定制处理器、FPGA 加速器和 AI 辅助硬件设计都能提供新的能效空间，但它们只有被编译器、硬件描述、综合优化、验证证据和软件栈连接起来，才能形成可迭代、可复用、可落地的系统能力。
...
本报告对“软硬件协同”的中心解释是：面向跨硬件架构的协同生态，编译与架构作为衔接软件需求和硬件实现的两个关键接口，连接应用需求、硬件能力、综合优化、验证证据和系统软件，使硬件设计创新不再停留于单点模块或单次生成，而能进入可重定向、可复用、可验证、可智能体能力提升的智能体能力支撑体系。
...
| 大模型与形式化技术驱动的软硬件协同方法 | EggMind（重点），OriGen（背景） | 大模型生成与搜索能力需要形式化语义、等价约束、可验证反馈和可控优化空间 | EggMind 作为现有工作重点展开；OriGen 只作为章节背景，支撑后续 IntelliC、Spine 与工业异构算子方向 |
...
1. 验证约束下的架构、硬件与编译协同生成：已有 AI 辅助 RTL 与硬件设计经验显示，智能体不能无约束生成，需要把设计意图、架构边界、RTL 证据和可执行判定基准组织为可相互校验的生成过程；Spine 可作为该方向已有基础或原型。Spine 不能只承接 OriGen，它还应承接 APS/Aquas 的端到端协同框架，作为 APS/Aquas 思想的智能体化延伸，把应用意图、架构接口、硬件生成、编译降低、可执行判定基准、IR、RTL、执行对齐和跨层检查放入统一验证约束。
2. 智能体编译基础设施：已有 MLIR、等价图和编译经验显示，未来编译器不仅要能优化，还要能给人和智能体提供可读、可验证、可追踪的语义与证据；IntelliC 可作为该方向已有基础或原型。`resources/compiler-infra-is-harness-medium.md` 的核心论证应被吸收：编译基础设施是智能体重定向的协作媒介，将目标事实、显式 IR、语义契约、受约束变换、后端产物和证据/反馈组织为可检查的生成边界。
3. 面向工业级异构架构的算子优化与智能体化编译协同：硬件和编译定制最终要落到工业级异构架构上的算子生成、优化、系统编排和性能剖析/执行轨迹；PTO 分布式执行特征可作为该方向已有基础或原型，`resources/seed-proposal.docx` 中的长程芯片算子自动优化智能体协作框架是该方向的重要未来工作来源。
