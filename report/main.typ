...
#figure(
  image("assets/cement/framework.pdf", width: 92%),
  caption: [Cement/CmtHDL 框架。源文将硬件类型、接口、模块连接、event layer 和控制子语言组织为周期确定型硬件前端，并由 CmtC 执行周期分析和有限状态机综合。],
)
...
#figure(
  image("assets/cement/shuffler.pdf", width: 90%),
  caption: [Cement 源文中的 shuffler 示例。该例展示了复杂流水控制、仲裁、数据重发和周期对齐为何需要在前端显式表达。],
)
...
#figure(
  image("assets/cement/state-tree.png", width: 72%),
...
#figure(
  image("assets/clay/framework.png", width: 92%),
...
#figure(
  image("assets/clay/schedule.png", width: 92%),
...
#figure(
  image("assets/skyegg/overview.pdf", width: 92%),
...
#figure(
  image("assets/skyegg/egraph.pdf", width: 92%),
...
#figure(
  image("assets/skyegg/speedup_ilp_vs_asap.pdf", width: 92%),
...
#figure(
  image("assets/eggmind/overview.pdf", width: 92%),
...
#figure(
  image("assets/eggmind/eqsatl.pdf", width: 92%),
...
#figure(
  image("assets/eggmind/proof-memory.pdf", width: 92%),
...
#figure(
  image("assets/eggmind/isaria-main-comparison.pdf", width: 90%),
...
#figure(
  image("assets/origen/code-to-code.pdf", width: 86%),
...
#figure(
  image("assets/origen/generation.pdf", width: 86%),
...
IntelliC 的未来研究重点正是构建这样一类编译基础设施。其基础设计将 IR 拆分为 `Sy + Se`：`Sy` 负责语法、结构、身份、验证、规范文本和解析，`Se` 负责由类型化语义定义、语义层级键和 TraceDB 承载的语义定义。这样的拆分避免把语义隐藏在解析器或编译过程的临时状态中，也避免把智能体的解释当作编译事实。一个操作可以有具体值、抽象范围、符号、后端证据等多个语义层级；同一个编译流程可以通过 TraceDB 记录匹配、修改意图、语义事实、诊断、义务和证据链接，从而让人和智能体都能追踪“为什么发生了这个变换、它依赖哪些事实、通过了哪些检查门、产生了哪些后端证据”。
