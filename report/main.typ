...
本人的研究目标是围绕算力紧缺背景下的高能效计算与敏捷芯片设计，构建以创新性编译技术为核心的软硬件协同方法体系。该目标可分解为三个相互衔接的技术层次：

+ 在领域定制与端到端协同层，研究可重定向编译、复杂 ISAX 映射、可复用自定义指令和微架构感知定制，使领域定制能力能够跨处理器接口、应用场景和工具链复用。
+ 在硬件前端与综合优化层，研究周期确定型硬件描述、综合中间表示和 e-graph 联合优化，提高从高层描述到高质量硬件实现的生产力。
+ 在生态与系统延伸层，研究面向人和 LLM agent 的可解释编译基础设施、验证约束下的软硬件协同生成、自动化优化策略和异构运行时协同，使上述方法进入可复用、可验证、可落地的系统闭环。
